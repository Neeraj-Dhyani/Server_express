...
async function SendOrdertoGmail(order){
...
function CancelOrder(order, approveLink){
   const transtport = nodemailer.createTransport({
    service:"gmail",
    auth:{
        user:"neerajdhyani47@gmail.com",
    ...
    <div style="font-family:Arial,Helvetica,sans-serif;background:#f6f8fb;padding:20px;">
      <div style="max-width:700px;margin:auto;background:#fff;border-radius:8px;box-shadow:0 0 10px rgba(0,0,0,0.05);overflow:hidden;">
        ...
        <div style="padding:20px;">
          <p>Hello Tarun,</p>
          <p>This Order is Cancel. Below are the details:</p>
            ...
            <tbody>
              ${order.products
                ...
                <tr>
                  ...
                  <td style="padding:8px;border-bottom:1px solid #eee;text-align:center;">${user.quantity}</td>
                  <td style="padding:8px;border-bottom:1px solid #eee;">₹${user.price}</td>
                </tr>`
                )
                .join("")}
            </tbody>
    ...
    <p>User <b>${order.customerName}</b> requested to cancel Order #${order._id}.</p>
        <p>Click below to approve cancellation:</p>
